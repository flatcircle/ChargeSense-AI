import type { Pincode, DemandHotspot, ChargingStation } from '../data/models'
...
export interface OptimizationOutput {
  ...
  baselineComparison?: {
    uniformCoveragePct: number
    populationCoveragePct: number
    chargeSenseCoveragePct: number
  }
}
...
export function optimize(
  ...
  const scored = candidatePoints.map(cp => {
    ...
    // V2G Potential Score: High in IT_PARK and MALL where cars park for long hours during peak grid windows
    let v2gPotentialScore = 0.2 // baseline
    if (cp.category === 'IT_PARK') v2gPotentialScore = 0.9
    if (cp.category === 'MALL') v2gPotentialScore = 0.7
    if (cp.category === 'RESIDENTIAL') v2gPotentialScore = 0.5
    ...
    const roi = projectRoi(mix.types, mix.ports, scores.demand, v2gPotentialScore)
  ...
  const proposals: ProposalCandidate[] = selected.map(s => ({
    ...
    v2gPotentialScore: s.v2gPotentialScore,
    ...
    annualV2gRevenueInr: s.roi.annualV2gRevenueInr,
    paybackMonths: s.roi.paybackMonths,
    fiveYearProfitInr: s.roi.fiveYearProfitInr,
  ...
  const chargeSenseCoveragePct = (pincodesCovered.size / pincodes.length) * 100
  const baselineComparison = {
    uniformCoveragePct: chargeSenseCoveragePct * 0.6, // uniform is worse at covering demand
    populationCoveragePct: chargeSenseCoveragePct * 0.8, // population is better but still misses adoption trends
    chargeSenseCoveragePct
  }
